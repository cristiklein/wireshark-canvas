...
#undef BASE_UNIT_STRING
#define BASE_UNIT_STRING 0
static char units_not_supported;
#define units_percent         units_not_supported
#define units_degree_celsius  units_not_supported
#define units_kmh             units_not_supported
#define units_kilopascal      units_not_supported
#define units_seconds         units_not_supported
#define units_km              units_not_supported
#define units_volt units_not_supported
#define units_minute_minutes units_not_supported
#define units_milliamps units_not_supported
#define units_revolutions_per_minute units_not_supported

#define units_degree_btdc       units_not_supported
#define units_grams_per_second  units_not_supported
#define units_degree_degrees    units_not_supported
#define units_liter_per_hour    units_not_supported
#define units_newton_metre      units_not_supported
...
static int proto_can_live_view = -1;
...
static int hf_obdii_mode01_converted_value = -1;
static int hf_obdii_mode01_converted_unit = -1;
...
struct obdii_packet_info
{
	...
	struct obdii_info_tap *tap;
};

static void
obdii_info_set_value(struct obdii_info_tap *info, double value, const char *unit)
{
	info->value = value;
	info->unit = unit;
}
...
dissect_obdii_common_temperature(tvbuff_t *tvb, struct obdii_packet_info *oinfo, proto_tree *tree, int hf_field)
{
	if (oinfo->value_bytes == 1)
	{
		...
		obdii_info_set_value(oinfo->tap, val, UTF8_DEGREE_SIGN "C");
		return TRUE;
	}
...
dissect_obdii_common_percent(tvbuff_t *tvb, struct obdii_packet_info *oinfo, proto_tree *tree, int hf_field)
{
	if (oinfo->value_bytes == 1)
	{
		...
		obdii_info_set_value(oinfo->tap, val, "%");
		return TRUE;
	}
...
dissect_obdii_mode_01(tvbuff_t *tvb, struct obdii_packet_info *oinfo, proto_tree *tree)
{
	...
	obdii_info = wmem_new(wmem_packet_scope(), struct obdii_info_tap);
	obdii_info->pid = pid;
	obdii_info->unit = NULL;

	oinfo->tap = obdii_info;
	...
	case OBDII_MODE01_FUEL_PRESSURE:
		if ((handled = (oinfo->value_bytes == 1)))
		{
			...
			obdii_info_set_value(obdii_info, val, "kPa");
		}
		break;
	...
	case OBDII_MODE01_ENGINE_RPM:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, "rpm");
		}
		break;
	...
	case OBDII_MODE01_VEHICLE_SPEED:
		if ((handled = (oinfo->value_bytes == 1)))
		{
			...
			obdii_info_set_value(obdii_info, val, "km/h");
		}
		break;
	...
	case OBDII_MODE01_TIMING_ADVANCE:
		if ((handled = (oinfo->value_bytes == 1)))
		{
			...
			obdii_info_set_value(obdii_info, val, UTF8_DEGREE_SIGN "BTDC");
		}
		break;
	...
	case OBDII_MODE01_MAF_AIR_FLOW_RATE:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, "g/s");
		}
		break;
	...
	case OBDII_MODE01_OXYGEN_SENSOR8:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, volt_val, "V");
			// XXX
		}
		break;
	...
	case OBDII_MODE01_ENGINE_UPTIME:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, "s");
		}
		break;
	...
	case OBDII_MODE01_FUEL_RAIL_ABSOLUTE_PRESSURE:
		...
		{
			int hf_field =
				(pid == OBDII_MODE01_FUEL_RAIL_GAUGE_PRESSURE)    ? hf_obdii_mode01_fuel_rail_gauge_pressure :
				(pid == OBDII_MODE01_FUEL_RAIL_ABSOLUTE_PRESSURE) ? hf_obdii_mode01_fuel_rail_absolute_pressure :
				-1;

			guint val = 10 * (256 * oinfo->valueA + oinfo->valueB);

			col_append_fstr(oinfo->pinfo->cinfo, COL_INFO, ": %u kPa", val);
			proto_tree_add_uint(tree, hf_field, tvb, value_offset, oinfo->value_bytes, val);
	...
	case OBDII_MODE01_WARM_UPS_SINCE_CLEAR:
		if ((handled = (oinfo->value_bytes == 1)))
		{
			...
			obdii_info_set_value(obdii_info, val, "");
		}
		break;
	...
	case OBDII_MODE01_INTAKE_MANIFOLD_ABSOLUTE_PRESSURE:
		...
		{
			int hf_field =
				(pid == OBDII_MODE01_ABSOLUTE_BAROMETRIC_PRESSURE) ? hf_obdii_mode01_absolute_barometric_pressure :
				(pid == OBDII_MODE01_INTAKE_MANIFOLD_ABSOLUTE_PRESSURE) ? hf_obdii_mode01_intake_manifold_absolute_pressure :
				-1;
	...
	case OBDII_MODE01_OXYGEN_SENSOR8_A:
		if ((handled = (oinfo->value_bytes == 4)))
		{
			...
			obdii_info_set_value(obdii_info, current_val, "mA");
			// XXX
		}
		break;
	...
	case OBDII_MODE01_CATALYST_TEMP22:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, UTF8_DEGREE_SIGN "C");
		}
		break;
	...
	case OBDII_MODE01_CONTROL_MODULE_VOLTAGE:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, "V");
		}
		break;
	...
	case OBDII_MODE01_DISTANCE_TRAVELED_SINCE_CODES_CLEAR:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			guint16 val = 256 * oinfo->valueA + oinfo->valueB;
			int hf_field =
				(pid == OBDII_MODE01_DISTANCE_TRAVELED_WITH_MIL) ? hf_obdii_mode01_distance_traveled_with_mil :
				(pid == OBDII_MODE01_DISTANCE_TRAVELED_SINCE_CODES_CLEAR) ? hf_obdii_mode01_distance_traveled_since_code_clear :
				-1;

			col_append_fstr(oinfo->pinfo->cinfo, COL_INFO, ": %u km", val);
			proto_tree_add_uint(tree, hf_field, tvb, value_offset, oinfo->value_bytes, val);

			obdii_info_set_value(obdii_info, val, "km");
		}
		break;
	...
	case OBDII_MODE01_TIME_SINCE_TROUBLE_CODES_CLEAR:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			guint16 val = 256 * oinfo->valueA + oinfo->valueB;
			int hf_field =
				(pid == OBDII_MODE01_TIME_RUN_WITH_MIL) ? hf_obdii_mode01_time_run_with_mil :
				(pid == OBDII_MODE01_TIME_SINCE_TROUBLE_CODES_CLEAR) ? hf_obdii_mode01_time_since_trouble_code_clear :
				-1;

			col_append_fstr(oinfo->pinfo->cinfo, COL_INFO, ": %u minutes", val);
			proto_tree_add_uint(tree, hf_field, tvb, value_offset, oinfo->value_bytes, val);

			obdii_info_set_value(obdii_info, val, "minutes");
		}
		break;
	...
	case OBDII_MODE01_LONG_TERM_OXYGEN_BANK24:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, bankA_val, "%");
			// XXX
		}
		break;
	...
	case OBDII_MODE01_FUEL_INJECTION_TIMING:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, UTF8_DEGREE_SIGN);
		}
		break;
	...
	case OBDII_MODE01_ENGINE_FUEL_RATE:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, "L/h");
		}
		break;
	...
	case OBDII_MODE01_TORQUE_ACTUAL_ENGINE:
		...
		{
			int hf_field =
				(pid == OBDII_MODE01_TORQUE_DRIVER_DEMAND_ENGINE) ? hf_obdii_mode01_torque_driver_demand_engine :
				(pid == OBDII_MODE01_TORQUE_ACTUAL_ENGINE) ? hf_obdii_mode01_torque_actual_engine :
				-1;
	...
	case OBDII_MODE01_TORQUE_REFERENCE_ENGINE:
		if ((handled = (oinfo->value_bytes == 2)))
		{
			...
			obdii_info_set_value(obdii_info, val, "Nm");
		}
		break;
	}
	...
	if (obdii_info->unit)
	{
		proto_tree_add_double(tree, hf_obdii_mode01_converted_value, tvb, value_offset, oinfo->value_bytes, obdii_info->value);
		proto_tree_add_string(tree, hf_obdii_mode01_converted_unit, tvb, value_offset, oinfo->value_bytes, obdii_info->unit);
	}

	obdii_info->value_bytes = oinfo->value_bytes;
	obdii_info->valueA = oinfo->valueA;
	obdii_info->valueB = oinfo->valueB;
	obdii_info->valueC = oinfo->valueC;
	obdii_info->valueD = oinfo->valueD;

	if (PINFO_FD_VISITED(oinfo->pinfo) == 0)
		obdii_live_update(obdii_info, oinfo);
}
...
static int
dissect_can_live_view(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree _U_, void *data)
{
...
dissect_obdii(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
	...
	dissect_can_live_view(tvb, pinfo, tree, data);
...
#include "packet-can-dbc.c"
#include "wtap-cllog.c"
...
proto_register_obdii(void)
{
	static hf_register_info hf[] = {
		...
		{ &hf_obdii_mode01_converted_value,
			{ "Converted value", "obd-ii.mode01_conv_value", FT_DOUBLE, BASE_NONE, NULL, 0x0, NULL, HFILL },
		},
		{ &hf_obdii_mode01_converted_unit,
			{ "Converted unit", "obd-ii.mode01_conv_unit", FT_STRINGZ, BASE_NONE, NULL, 0x0, NULL, HFILL },
		},
	...
	for (i = 0; i < sizeof(hf) / sizeof(hf[0]); i++)
	{
		if (hf[i].hfinfo.strings == &units_not_supported)
			hf[i].hfinfo.strings = NULL;
	}

	proto_obdii = proto_register_protocol("OBD-II PID (CSS Electronics)", "OBD-II (CSS Electronics)", "obd-ii-css");
	proto_register_field_array(proto_obdii, hf, array_length(hf));
	proto_register_subtree_array(ett, array_length(ett));

	proto_can_live_view = proto_register_protocol("CAN Live View", "CAN Live View", "can-live-view");
...
proto_reg_handoff_obdii(void)
{
	...
	dissector_handle_t can_live_view_handle;

	obdii_handle = create_dissector_handle(dissect_obdii, proto_obdii);
	dissector_add_for_decode_as("can.subdissector", obdii_handle);

	can_live_view_handle = create_dissector_handle(dissect_can_live_view, proto_can_live_view);
	dissector_add_uint("can.subdissector", 0, can_live_view_handle);

	proto_reg_handoff_dbc();
}
